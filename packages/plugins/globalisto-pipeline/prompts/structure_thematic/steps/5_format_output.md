# Step 5: Format Output for Retention Structure
...
Transform thematic structure into the format expected by `retention_structure.md`.

This is an ADAPTER step. It combines:
...
And outputs `candidate_units.yaml` matching the input schema that `retention_structure.md` expects.
...
**BEAT_GRAPH.yaml:**
- [ ] All beats have timing, function, and dependencies
- [ ] production_sections exist and cover all beats
- [ ] Section count is 3-4 (never fewer than 3, never more than 5)
- [ ] Every beat appears in exactly one production_section
- [ ] HOOK section is first (section_id: 1)
- [ ] LAND section is last
- [ ] Section timing is contiguous (no gaps)
- [ ] Each section has mood derived from emotional_arc
- [ ] Section names match their function (HOOK, BUILD, ESCALATE, LAND)
...
## Additional Output: BEAT_GRAPH.yaml with Production Sections

After generating `candidate_units.yaml`, you must also output `BEAT_GRAPH.yaml` which includes:
1. The complete beat sequence (generated from retention structure design)
2. **NEW:** `production_sections` that group beats for music cues and midroll placement

### Production Sections Schema

Production sections group beats into 3-4 larger units based on:
- **Function transitions** (HOOK → GROUND → PROVE → IMPLICATE → LAND)
- **Emotional arc phases** (from retention_design.emotional_arc)
- **Timing intervals** (~2-4 minutes per section)
...
production_sections:
  ...
  - section_id: 2
    name: "BUILD"
    mood: "grief → revelation"
    beats: ["BEAT_03", "BEAT_04", "BEAT_05"]
    timing: "0:30-3:30"

  - section_id: 3
    name: "ESCALATE"
    mood: "outrage → absurdity"
    beats: ["BEAT_06", "BEAT_07", "BEAT_08", "BEAT_09", "BEAT_10"]
    timing: "3:30-7:30"

  - section_id: 4
    name: "LAND"
    mood: "confrontation → urgency"
    beats: ["BEAT_11", "BEAT_12", "BEAT_13", "BEAT_14"]
    timing: "7:30-10:00"
```

### Derivation Rules for Production Sections

**Rule 1: HOOK Section (0:00-0:30)**
- All beats with function="HOOK" or timing in first 30 seconds
- Mood: Derived from first beat's emotional register (usually "shock", "intrigue", or "tension")
- Purpose: Grab attention, establish stakes

**Rule 2: BUILD Section (~0:30-3:30)**
- From end of HOOK through GROUND and initial PROVE beats
- Typically includes first victim/human story (GROUND) and setup evidence (PROVE)
- Mood: Transition from hook emotion → deeper emotional engagement (e.g., "grief → revelation")
- Purpose: Establish emotional foundation and begin evidence accumulation

**Rule 3: ESCALATE Section (~3:30-7:30)**
- Main PROVE arc with IMPLICATE beats
- Where evidence accumulates, absurdity escalates, viewer recognition happens
- Mood: Usually escalating emotions (e.g., "outrage → absurdity", "revelation → recognition")
- Purpose: Peak of information delivery and emotional intensity

**Rule 4: LAND Section (~7:30-10:00)**
- Final GROUND (if any), all LAND beats, resolution
- Where consequences are revealed, "where are they now", final confrontation
- Mood: Shift to weight, stillness, urgency, or dark irony
- Purpose: Emotional resolution, call to awareness, lasting impact

### Alternative: Split ESCALATE if Arc is Long

For videos >12 minutes or with complex arcs, you may split ESCALATE into two sections:
- **ESCALATE_1**: Initial PROVE phase (revelation)
- **ESCALATE_2**: Peak PROVE + IMPLICATE (confrontation)

### Mood Derivation

Mood should be a short phrase (2-4 words) capturing the emotional journey within that section:
- Use `→` to show transitions (e.g., "grief → revelation")
- Draw from emotional_arc in retention_design
- Match the emotional_register of beats in that section

Examples:
- "shock" (single emotion, no transition)
- "grief → anger" (clear transition)
- "revelation → outrage → absurdity" (multi-stage progression)

### Timing Format

- Use MM:SS format (e.g., "3:30-7:30")
- Derive from beat_sequence.timing fields
- Start of section = start of first beat
- End of section = end of last beat

### Process for Generating Production Sections

1. **Review beat sequence** in BEAT_GRAPH.yaml
2. **Identify function boundaries**:
   - End of HOOK zone (usually after first 30 seconds)
   - First GROUND beat (starts BUILD)
   - First IMPLICATE beat (may mark transition to ESCALATE)
   - First LAND beat (starts LAND section)
3. **Group beats** based on:
   - Functional coherence (keep similar functions together)
   - Timing (aim for 2-4 minute sections)
   - Emotional progression (section should have coherent mood arc)
4. **Name sections** based on their role:
   - "HOOK" - always for opening
   - "BUILD" - establishing foundation
   - "ESCALATE" - peak intensity (can be "ESCALATE_1", "ESCALATE_2" if split)
   - "LAND" - resolution and consequences
5. **Derive mood** from emotional_arc and beat emotional_registers
6. **Set timing** from beat timing ranges

### Example: 10-Minute Video with 14 Beats
...
**Reasoning:**
- HOOK: First 30 seconds, shock value
- BUILD: GROUND beat + early PROVE (3 minutes to establish context)
- ESCALATE: Main PROVE + IMPLICATE arc (4 minutes of peak content)
- LAND: Final GROUND + all LAND beats (2.5 minutes of resolution)

### Hard Constraints

- **Must have 3-4 sections** (never fewer than 3, never more than 5)
- **HOOK section always first** (section_id: 1)
- **LAND section always last**
- **No gaps**: Every beat must belong to exactly one section
- **Timing must be contiguous**: End of section N = start of section N+1
- **Section duration**: Minimum 1 minute, maximum 5 minutes per section

### Integration in BEAT_GRAPH.yaml

Add `production_sections` as a top-level key in BEAT_GRAPH.yaml, after `retention_design`:
...
## Enrichment Pool in BLUEPRINT.md

If `STEP2_selected_themes.yaml` contains an `enrichment_pool` section, include it as an appendix in `BLUEPRINT.md` under the heading:

```markdown
## Enrichment Material Available (Not Structurally Necessary)
```

For each enrichment pool item, list:
- Its ID
- Its `audience_value` tag
- Its `one_liner`

After the item list, include this guidance block verbatim:
...
This appendix is informational only. Enrichment items are NOT units. The hard constraint "Do NOT add units beyond themes + narrative_material" still applies — enrichment items exist solely as writer-phase texture that may improve audience comprehension.

If `STEP2_selected_themes.yaml` has no `enrichment_pool` section, omit this appendix entirely.
...
- `BEAT_GRAPH.yaml` complete with production_sections
...
After this step completes, the structure module outputs are consumed by the Writing module:
- `BEAT_GRAPH.yaml` (with production_sections) → music generation, midroll placement
- `BLUEPRINT.md` → content execution

The `production_sections` will be used by:
1. `scripts/calculate_music_durations.js` - Maps sections to music types
2. `packaging/midroll_placement.md` - Places ad breaks at section boundaries
