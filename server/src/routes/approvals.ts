...
export function approvalRoutes(
  ...
  /**
   * Returns the agent only if it belongs to the given company.
   * Logs a warning and returns null if missing or cross-company.
   */
   ...
   * agent ever sees it. evaluateQueuedRunStaleness also cancels a queued run
   * whose anchor issue is assigned to a DIFFERENT agent than the run's, so the
   * anchor must be both non-terminal AND owned by the agent being woken. If no
   * linked issue qualifies we return null so the wake is agent-level (no issue
   * anchor) and always survives the gate.
   */
  function pickNonTerminalAnchorIssueId(
    ...
    const live = linkedIssues.find(
      (i) => !TERMINAL.has(i.status) && i.assigneeAgentId === wakeAgentId,
    );
    return live?.id ?? null;
  }
  ...
  router.post("/approvals/:id/approve", validate(resolveApprovalSchema), async (req, res) => {
    ...
    if (applied) {
      ...
      if (approval.requestedByAgentId) {
        const requesterAgent = await resolveRequesterAgent(
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      // Wake the card creator so the single-card request-revision -> resubmit cycle
      // self-drives. Mirrors the approval_approved wake; carries decisionNote so the
      // agent knows what to rework. (This route previously logged activity only — the
      // missing wake is why the revision cycle could not self-drive.)
      if (approval.requestedByAgentId) {
        const requesterAgent = await resolveRequesterAgent(
          approval.requestedByAgentId,
          approval.companyId,
        ...
        if (requesterAgent) {
          try {
            ...
            logger.warn(
              ...
              "failed to queue requester wakeup after request-revision",
            );
            await logActivity(db, {
              companyId: approval.companyId,
              actorType: "user",
